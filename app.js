  ...
  document.getElementById('assess-impact-list').innerHTML = IMPACT_MODULES.map((m,i) => `
    <div class="assess-impact-card" onclick="showImpactInfo('${m.id}', ${i})">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:var(--muted);margin-top:1px">${m.questions.length} questions · Likert scale</div>
      </div>
      <div class="assess-card-chevron">›</div>
    </div>`).join('');
}
...
function showImpactInfo(moduleId, idx){
  ...
  document.getElementById('explainBody').innerHTML = `
    <p style="margin-bottom:10px;font-size:13px;line-height:1.6">Assesses how digital device use is affecting your ${m.name.toLowerCase()} health. ${m.questions.length} questions on a 5-point Likert scale.</p>
    ${scoreHtml}
...
function renderAboutScales(){
  document.getElementById('aboutScaleList').innerHTML =
    ...
    IMPACT_MODULES.map(m => `<div style="margin-bottom:8px;padding:8px 0;border-bottom:1px solid var(--border)"><div style="font-size:12px;font-weight:700;color:var(--text)">${m.icon} ${m.name} Impact Module</div><div style="font-size:11px;color:var(--muted);margin-top:2px">Custom validated-style module · ${m.questions.length} items · 5-point Likert scale</div></div>`).join('');
}
